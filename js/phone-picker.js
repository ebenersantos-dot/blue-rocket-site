/**
 * Blue Rocket Co. — Phone country-code picker (React via CDN)
 *
 * Rendered into any element with data-react="phone-picker".
 * Exposes window.BRGetPhone() → { code, number, full } for the form handler.
 *
 * Requires React 18 + ReactDOM loaded before this file (see HTML).
 */
...
(function () {
  ...
  /* ─── Country data (code, dial, flag emoji) ──────────────────────── */
  const COUNTRIES = [
  ...
  /* Default to Spain (+34) since that's where Blue Rocket Co. is based */
  ...
  function PhonePicker({ onChange }) {
    ...
    useEffect(() => {
      if (onChange) {
        onChange({
          code:   selected.dial,
          number: number,
          full:   number ? selected.dial + number.replace(/\s/g, '') : ''
        });
      }
    }, [selected, number]);
    ...
    const hasError = touched && number.trim() !== '' &&
      !/^\d[\d\s\-().]{5,18}$/.test(number.trim());

    return React.createElement('div', { className: 'phone-picker', ref: dropdownRef },
      ...
      React.createElement('button', {
        ...
        React.createElement('span', { className: 'phone-flag', 'aria-hidden': 'true' }, selected.flag),
        React.createElement('span', { className: 'phone-dial' }, selected.dial),
        React.createElement('span', { className: 'phone-chevron' + (open ? ' is-open' : ''), 'aria-hidden': 'true' }, '▾')
      ),
      ...
      open && React.createElement('div', {
        ...
        React.createElement('div', { className: 'phone-search-wrap' },
        ...
        React.createElement('ul', { className: 'phone-list' },
          filtered.length === 0
            ? React.createElement('li', { className: 'phone-list-empty' }, 'No results')
            : filtered.map(c =>
                React.createElement('li', {
                  key: c.code,
                  role: 'option',
                  'aria-selected': selected.code === c.code,
                  className: 'phone-option' + (selected.code === c.code ? ' is-selected' : ''),
                  onClick: () => selectCountry(c),
                },
                  React.createElement('span', { className: 'phone-flag', 'aria-hidden': 'true' }, c.flag),
                  React.createElement('span', { className: 'phone-option-name' }, c.name),
                  React.createElement('span', { className: 'phone-option-dial' }, c.dial)
                )
              )
        )
      ),
      ...
      hasError && React.createElement('span', { className: 'phone-error form-error' },
  ...
  document.querySelectorAll('[data-react="phone-picker"]').forEach(function (el) {
    ...
    let hiddenInput = formField
      ? formField.querySelector('input[type="hidden"][name="' + inputName + '"]')
      : null;

    if (!hiddenInput && formField) {
